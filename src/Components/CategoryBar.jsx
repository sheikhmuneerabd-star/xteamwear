...
import topCategoriesData12 from "../topCategoriesData12";
import topCategoriesData34 from "../topCategory34";
import topCategoriesData56 from "../topCategories56";
    ...
    <div>
        <div className="w-full categoryBackground hidden xl:flex">
            <div className="w-[92%] h-[47px] mx-auto flex justify-between items-center category">
                        ...
                        <div className="flex flex-col gap-4">
                            {category.map((item) => (
                                <div key={item.id} className="relative group/item w-[225px]">
                                    <Link to={`/categorySection/${item.name}`}>
                ...
                <div className="flex h-full items-center gap-5 font-semibold text-[15px] mr-2">
                    ...
                    {
                        topCategoriesData12.map((cate, index) => (
                            <div key={cate.id} className="relative h-full flex items-center group cursor-pointer" onMouseEnter={(e) => {enter(e), setDropdownOpen(true)}} onMouseLeave={(e) => {leave(e), setDropdownOpen(false)}}>
                                ...
                                <div className={`w-[260px] z-50 h-fit pointer-events-none opacity-0 ${dropdownOpen ? 'group-hover:opacity-100' : 'opacity-0'} -translate-y-3 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-200 bg-white border shadow-md absolute left-[-10px] top-[45px] px-5 py-2`}>
                                    {cate.submenus.map((subCate) => (
                                        <div key={subCate.id} className="mt-2 group/subItem">
                                            <span className="cursor-pointer text-sm relative inline-block text-gray-600 group-hover/subItem:text-gray-900" onMouseEnter={(e) => enter(e)} onMouseLeave={(e) => leave(e)}>
                                                {subCate.name}
                                                <span className="line absolute left-0 top-[19px] h-[1.7px] w-full"></span>
                                            </span>
                                            {index !== cate.submenus.length - 1 && <div className="w-full h-[1px] bg-gray-100 mt-[10px]"></div>}
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div key={cate.id} className="relative h-full flex items-center group cursor-pointer" onMouseEnter={(e) => {enter(e), setDropdownOpen(true)}} onMouseLeave={(e) => {leave(e), setDropdownOpen(false)}}>
                                ...
                                <div className={`w-[260px] z-50 flex flex-col gap-3 h-fit pointer-events-none opacity-0 ${dropdownOpen ? 'group-hover:opacity-100' : 'opacity-0'} -translate-y-3 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-200 bg-white border shadow-md absolute left-[-10px] top-[45px] px-5 py-2`}>
                                    {cate.submenus.map((subCate, index) => (
                                        <div key={subCate.id}>
                                            <div className={`relative ${index !== cate.submenus.length - 1 ? "mb-3" : "mb-1"} mt-1 group/item w-[235px]`}>
                                                <div className="flex items-center justify-between group font-medium cursor-pointer">
                                                    <span className="relative text-gray-700 group-hover:text-gray-950" onMouseEnter={(e) => enter(e)} onMouseLeave={(e) => leave(e)}>
                                                        {subCate.name}
                                                        <span className="line absolute left-0 bottom-[1px] h-[1.6px] w-full"></span>
                                                    </span>
                                                    {subCate.item && <RiArrowRightSLine className="absolute left-[213px] top-[4px] text-lg text-gray-500" />}
                                                </div>
                                                {subCate.item && (
                                                    <div className="w-[260px] h-fit pointer-events-none opacity-0 group-hover/item:opacity-100 -translate-y-5 group-hover/item:translate-y-0 group-hover/item:pointer-events-auto transition-all duration-200 bg-white border shadow-md absolute left-[235px] -top-[8px] px-5 py-2">
                                                        {subCate.item.map((subCate2, index) => (
                                                            <div key={index} className={`cursor-pointer mt-2 group/subItem ${index !== subCate.item.length - 1 ? "mb-0" : "mb-1"}`}>
                                                                ...
                                                                {index !== subCate.item.length - 1 && <div className="w-full h-[1px] bg-gray-100 mt-[10px]"></div>}
                                                            </div>
                                                        ))}
                                                    </div>
                                                )}
                                            </div>
                                            {index !== cate.submenus.length - 1 && <div className="w-full h-[2px] bg-gray-100"></div>}
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    {topCategoriesData56.map((cate, index) => (
                        <div key={cate.id} className="relative h-full flex items-center group cursor-pointer" onMouseEnter={(e) => {enter(e), setDropdownOpen(true)}} onMouseLeave={(e) => {leave(e), setDropdownOpen(false)}}>
                            <span>{cate.title}</span>
                            <span className="line absolute left-0 top-[34px] h-[2px] w-full"></span>
                            <div className={`w-[260px] z-50 h-fit pointer-events-none opacity-0 ${dropdownOpen ? 'group-hover:opacity-100' : 'opacity-0'} -translate-y-3 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-200 bg-white border shadow-md absolute ${cate.id === 2 ? "-left-[60px]" : "left-[-10px]"} top-[45px] px-5 py-2`}>
                                {cate.submenus.map((subCate) => (
                                    <div key={subCate.id} className="mt-2 group/subItem">
                                        <span className="cursor-pointer text-sm relative inline-block text-gray-600 group-hover/subItem:text-gray-900" onMouseEnter={(e) => enter(e)} onMouseLeave={(e) => leave(e)}>
                                            {subCate.name}
        ...
        <div className={`fixed top-0 left-0 right-0 z-50 bg-yellow-400 hidden xl:flex justify-between h-[80px] px-4 items-center transition-all duration-150 ${cateShow ? "translate-y-0" : "-translate-y-full"}`}>
            ...
            <div className="flex flex-wrap items-center gap-5 font-semibold text-[15px] mr-2">
                ...
                </div>
                {
                    topCategoriesData12.map((cate, index) => (
                        <div key={cate.id} className="relative h-full flex items-center group cursor-pointer" onMouseEnter={(e) => {enter(e), setDropdownOpen(true)}} onMouseLeave={(e) => {leave(e), setDropdownOpen(false)}}>
                            <span>{cate.title}</span>
                            <span className="line absolute left-0 top-[24px] h-[2px] w-full"></span>
                            <div className={`w-[260px] z-50 h-fit pointer-events-none opacity-0 ${dropdownOpen ? 'group-hover:opacity-100' : 'opacity-0'} -translate-y-3 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-200 bg-white border shadow-md absolute left-[-10px] top-[28px] px-5 py-2`}>
                                {cate.submenus.map((subCate) => (
                                    <div key={subCate.id} className="mt-2 group/subItem">
                                        <span className="cursor-pointer text-sm relative inline-block text-gray-600 group-hover/subItem:text-gray-900" onMouseEnter={(e) => enter(e)} onMouseLeave={(e) => leave(e)}>
                                            {subCate.name}
                ...
                }
                {
                    topCategoriesData34.map((cate) => (
                        <div key={cate.id} className="relative h-full flex items-center group cursor-pointer" onMouseEnter={(e) => {enter(e), setDropdownOpen(true)}} onMouseLeave={(e) => {leave(e), setDropdownOpen(false)}}>
                            <span>{cate.title}</span>
                            <span className="line absolute left-0 top-[24px] h-[2px] w-full"></span>
                            <div className={`w-[260px] z-50 flex flex-col gap-3 h-fit pointer-events-none opacity-0 ${dropdownOpen ? 'group-hover:opacity-100' : 'opacity-0'} -translate-y-3 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-200 bg-white border shadow-md absolute left-[-10px] top-[28px] px-5 py-2`}>
                                {cate.submenus.map((subCate, index) => (
                                    <div key={subCate.id}>
                                        <div className={`relative ${index !== cate.submenus.length - 1 ? "mb-3" : "mb-1"} mt-1 group/item w-[235px]`}>
                                            <div className="flex items-center justify-between group font-medium cursor-pointer">
                ...
                {topCategoriesData56.map((cate, index) => (
                    <div key={cate.id} className="relative h-full flex items-center group cursor-pointer" onMouseEnter={(e) => {enter(e), setDropdownOpen(true)}} onMouseLeave={(e) => {leave(e), setDropdownOpen(false)}}>
                        <span>{cate.title}</span>
                        <span className="line absolute left-0 top-[24px] h-[2px] w-full"></span>
                        <div className={`w-[260px] z-50 h-fit pointer-events-none opacity-0 ${dropdownOpen ? 'group-hover:opacity-100' : 'opacity-0'} -translate-y-3 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-200 bg-white border shadow-md absolute ${cate.id === 2 ? "-left-[60px]" : "left-[-10px]"} top-[28px] px-5 py-2`}>
                            {cate.submenus.map((subCate) => (
                                <div key={subCate.id} className="mt-2 group/subItem">
                                    <span className="cursor-pointer text-sm relative inline-block text-gray-600 group-hover/subItem:text-gray-900" onMouseEnter={(e) => enter(e)} onMouseLeave={(e) => leave(e)}>
                                        {subCate.name}
            ...
            <Link to="/cart" className="relative">
                ...
                <span className='absolute -top-[9px] -right-[6px] flex items-center justify-center bg-red-500 rounded-full font-semibold text-white text-[14.5px] w-[25px] h-[25px]'>{cart.length}</span>
            </Link>
        </div>  
    </div>
  )
